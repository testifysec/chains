...
func setupEnv(taskRunName string, cfg config.Config, archClient *archivistaClient.ArchivistaClient) (*Backend, objects.TektonObject, *fakePipelineClient.Clientset) {
	tr := &v1beta1.TaskRun{
	...
	aStorage, err := NewStorageBackend(cfg, fakeClient)
	if err != nil {
